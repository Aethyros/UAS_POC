...
export default function AssessScreen({ targets }) {
  const tgt1 = targets.find(t => t.id === 'TGT-001');
  const tgt2 = targets.find(t => t.id === 'TGT-002');

  // Dynamically calculate proximity percentage based on incoming simulation data
  const proximityPercent = Math.min(100, Math.floor(92 + (380 - tgt1.distanceToHQ) * 0.05));
  ...
  return (
    <div style={{ padding: '30px', display: 'flex', flexDirection: 'column', gap: '20px', overflowY: 'auto', flex: 1 }}>
      ...
      <div style={{ display: 'flex', gap: '20px' }}>
        <div style={{ ...cardStyle, flex: 1.5 }}><p style={{ margin: '0 0 5px 0', color: theme.textSub }}>TGT-001 threat score</p><div><span style={{ fontSize: '2.5rem', color: theme.accentRed, fontWeight: 'bold' }}>{tgt1.threatScore}</span><span style={{ color: theme.textSub }}> / 100 · Tier 1 critical</span></div></div>
        <div style={{ ...cardStyle, flex: 1.5 }}><p style={{ margin: '0 0 5px 0', color: theme.textSub }}>TGT-002 threat score</p><div><span style={{ fontSize: '2.5rem', color: theme.accentYellow, fontWeight: 'bold' }}>{tgt2.threatScore}</span><span style={{ color: theme.textSub }}> / 100 · Tier 2 monitor</span></div></div>
      ...
      <div style={{ display: 'flex', gap: '20px' }}>
        <div style={{ ...cardStyle, flex: 1.5 }}>
          ...
          <div style={{ marginBottom: '15px' }}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}><span>Proximity to protected zone <span style={badgeStyle('Error')}>Critical</span></span><span>{proximityPercent}%</span></div><div style={{ height: '6px', backgroundColor: '#2A3441', borderRadius: '3px' }}><div style={{ height: '100%', width: `${proximityPercent}%`, backgroundColor: theme.accentRed, borderRadius: '3px', transition: 'width 0.5s ease' }}></div></div></div>
        ...
        <div style={{ ...cardStyle, flex: 1 }}>
          ...
          <div style={{ fontSize: '0.9rem', display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <div style={{ display: 'flex', gap: '15px' }}><span style={{ color: theme.textSub }}>14:23:14</span><div style={{ width: '30px', height: '30px', borderRadius: '50%', backgroundColor: 'rgba(229, 57, 53, 0.2)', color: theme.accentRed, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{tgt1.threatScore}</div><span style={{ flex: 1 }}>Score escalated to {tgt1.threatScore} — Tier 1 critical alert triggered</span></div>
